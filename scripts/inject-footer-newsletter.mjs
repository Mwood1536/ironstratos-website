// Inject footer newsletter widget (CSS + HTML + JS) into every page
// with a <footer> + .footer-grid. Idempotent.
//
// Node handles UTF-8 cleanly by default; this replaces an earlier
// PowerShell script that double-encoded em-dashes.
...
for (const rel of pages) {
  ...
  if (content.includes("footer-newsletter")) {
    console.log(`SKIP already-injected: ${rel}`);
    continue;
  }

  // 1) Inject CSS just before the closing </style>
  const idxStyle = content.indexOf("</style>");
  if (idxStyle < 0) { console.log(`SKIP no </style>: ${rel}`); continue; }
  content = content.slice(0, idxStyle) + css + content.slice(idxStyle);

  // 2) Insert HTML just before <div class="footer-grid">
  const needle = '<div class="footer-grid">';
  const idxGrid = content.indexOf(needle);
  if (idxGrid < 0) { console.log(`SKIP no .footer-grid: ${rel}`); continue; }
  content = content.slice(0, idxGrid) + html + content.slice(idxGrid);

  // 3) Append JS just before </body>
  const idxBody = content.lastIndexOf("</body>");
  if (idxBody < 0) { console.log(`SKIP no </body>: ${rel}`); continue; }
  content = content.slice(0, idxBody) + js + content.slice(idxBody);
